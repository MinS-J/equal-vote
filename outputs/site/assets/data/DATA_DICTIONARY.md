...
- `history[].edgeAdjacent`: SGIS 읍면동 경계에서 실제 경계선을 공유하는 같은 시군구 안의 pair 중 strict 동일득표 pair 수. 사이트 화면에서는 `경계 인접`으로 표시한다.
...
- `geoAdjacent`: 사이트의 `경계 인접` 범위에 쓰는 원천 집계. SGIS `bnd_dong` 읍면동 경계로 실제 경계선을 공유하는 동만 골라 센 결과이며, `equal_edge_same_sigungu`는 같은 시군구 안에서 실제 경계 인접인 pair 중 strict 동일득표 pair 수를 뜻한다.
- `jointProbabilities.models`: `A=분할동 1쌍 이상`, `B=광역권 8쌍 이상`을 같은 200,000회 반복 안에서 직접 센 결합사건 결과
- `jointProbabilities.adjacentModels`: `A=경계 인접 1쌍 이상`, `B=광역권 8쌍 이상`을 같은 200,000회 반복 안에서 직접 센 결합사건 결과
...
사이트 화면에서는 이 결과를 `분할동` 다음 단계인 `경계 인접` 비교 범위로 표시한다. 이름 기반 분할동 기준과 실제 경계 인접 기준은 서로 다른 질문이므로, 두 값을 나란히 두고 해석한다.
